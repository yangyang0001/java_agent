...
import deepblue.inaction_00_websocket.MultiModeWebSocketHandler;
...
public class WebSocketConfig implements WebSocketConfigurer {
    ...
    // MultiModeWebSocketHandler 现在是 @Component，需要维护在线连接的 Map，
    // 交给 Spring 管理成单例，而不是每次都 new 一个新的（否则每个连接都会有自己独立的在线列表）
    @Resource
    private MultiModeWebSocketHandler multiModeWebSocketHandler;
    ...
    public void registerWebSocketHandlers(WebSocketHandlerRegistry registry) {
        ...
        registry.addHandler(multiModeWebSocketHandler, "/ws/echo", "/ws/process", "/ws/broadcast", "/ws/unicast")
                .setAllowedOrigins("*");
    }
}
