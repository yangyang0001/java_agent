...
执行 `SimpleRNN.java` 里的 `main` 方法（`javac SimpleRNN.java && java
deepblue.inaction_01_hello_agents.chapter_03.SimpleRNN`），复现 §4 手算例子，实际
输出如下：

```text
=== §4 手算例子对照（期望 h1≈[0.565,-0.430] y1≈0.703，h2≈[0.150,0.081] y2≈0.106，h3≈[0.372,-0.195] y3≈0.427）===
t=1  x_t= 0.90  h_t=[0.565, -0.430]  y_t=0.703
t=2  x_t=-0.60  h_t=[0.150, 0.081]  y_t=0.107
t=3  x_t= 0.40  h_t=[0.372, -0.195]  y_t=0.428
forward() 方法返回的 y 序列: [[0.7034802138335452], [0.10676700499034714], [0.4275707392022591]]
```

逐步打印的 `h_t`、`y_t` 和 §4 手算表格里的数字一致（`y_t` 相差 0.001 是因为手算时
每一步都把 `h_t` 四舍五入到 3 位小数再代入下一步，代码里则是全精度浮点数传递）；
最后一行调用封装好的 `forward()` 方法，验证它和手动展开的计算结果完全吻合。
